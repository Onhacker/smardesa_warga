...
<head>
    ...
    <link rel="stylesheet" href="<?= base_url('assets/css/community.min.css') ?>?v=13">
...
<script>window.SDW={baseUrl:<?= json_encode(base_url()) ?>,csrfName:<?= json_encode($this->security->get_csrf_token_name()) ?>,csrfHash:<?= json_encode($this->security->get_csrf_hash()) ?>,serviceWorkerUrl:<?= json_encode(base_url('service-worker.js') . '?v=15') ?>,serviceWorkerScope:<?= json_encode(base_url()) ?>};</script>
