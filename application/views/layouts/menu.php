...
$menuMethod = strtolower((string) $this->router->fetch_method());
$menuLettersActive = nav_is(array('layanan', 'permohonan')) || (nav_is('dashboard') && $menuMethod === 'letters');
$menuAnnouncementsActive = nav_is('community') && in_array($menuMethod, array('announcements', 'announcement'), TRUE);
$menuComplaintsActive = nav_is('community') && in_array($menuMethod, array('complaints', 'complaint'), TRUE);
$menuContactActive = nav_is('community') && $menuMethod === 'contact';
$menuTokokuActive = nav_is('marketplace') && in_array($menuMethod, array('tokoku', 'store_settings', 'create', 'edit'), TRUE);
$menuMarketActive = nav_is('marketplace') && !$menuTokokuActive;
$menuAccountActive = nav_is('account');
$menuNotificationsActive = nav_is('notifications');
?>
...
<?php if ($staffMode): ?>
<h6 class="menu-divider mt-4">Pelayanan <?= e($institutionLabel) ?></h6>
...
<h6 class="menu-divider mt-4">Layanan</h6>
<div class="list-group list-custom-small list-menu warga-menu-list">
...
<div class="list-group list-custom-small list-menu warga-menu-list">
    <?php if ($menuIsAuthenticated && $menuCanManageMarketplace): ?><a class="<?= $menuTokokuActive ? 'active-nav' : '' ?>" href="<?= site_url('pasar/tokoku') ?>"><i class="fa fa-store-alt warga-menu-icon is-blue"></i><span>Tokoku</span><i class="fa fa-angle-right"></i></a><?php endif; ?>
    <?php if ($menuIsAuthenticated): ?><a class="<?= $menuAccountActive ? 'active-nav' : '' ?>" href="<?= site_url('akun') ?>"><i class="fa fa-user warga-menu-icon is-indigo"></i><span>Akun Saya</span><i class="fa fa-angle-right"></i></a><?php else: ?><a href="<?= site_url('login') ?>"><i class="fa fa-sign-in-alt warga-menu-icon is-blue"></i><span>Login</span><i class="fa fa-angle-right"></i></a><?php endif; ?>
    <a class="<?= $menuContactActive ? 'active-nav' : '' ?>" href="<?= site_url('kontak') ?>"><i class="fa fa-address-book warga-menu-icon is-teal"></i><span>Kontak <?= e($menuInstitution) ?></span><i class="fa fa-angle-right"></i></a>
