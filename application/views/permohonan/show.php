...
<section class="warga-result-band"><span><i class="fa fa-check"></i></span><div><strong>Surat telah diterbitkan</strong><p>Surat resmi mengikuti tampilan surat <?= e($institutionLower) ?> dan siap dilihat di sini.</p></div>
    <?php if ($officialDocumentFormat === 'html'): ?>
        <div class="warga-result-actions"><button type="button" class="btn btn-s bg-green-dark color-white rounded-s" data-warga-letter-open data-html-url="<?= e(site_url('permohonan/' . rawurlencode($request['id']) . '/surat-html')) ?>" data-html-name="<?= e('surat-' . ($request['local_reference'] ?? 'resmi') . '.html') ?>" data-pdf-url="<?= e(site_url('permohonan/' . rawurlencode($request['id']) . '/surat-pdf')) ?>" data-pdf-name="<?= e('surat-' . ($request['local_reference'] ?? 'resmi') . '.pdf') ?>"><i class="fa fa-eye"></i><span>Lihat Surat</span></button></div>
